...
export async function handleAmazonImport(request: Request, env: Env): Promise<Response> {
  ...
  for (const order of aggregated) {
    ...
    await env.DB.prepare(
      ...
      order.totalAmount,
      order.quantityTotal,
      JSON.stringify(order.productNames),
      JSON.stringify(order.sellerNames),
      order.orderStatus,
      order.paymentInstrumentType,
      order.shipTo,
    ...
    if (classification) {
      await env.DB.prepare(
        'DELETE FROM classifications WHERE transaction_id = ?',
      ).bind(match.transactionId).run();
    }
  ...
  return jsonOk({
    ...
    transactions_reclassified: reclassified,
    message: matched
