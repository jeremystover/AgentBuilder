...
import { handleMcp, type JsonRpcMessage } from "./mcp";
...
// JSON-RPC 2.0 endpoint that exposes a small set of channel-manager
// tools (list units / check availability / pull calendars / etc.) to
// Claude as a custom connector. Bearer auth via MCP_HTTP_KEY secret.
app.post("/mcp", async (c) => {
  const expected = c.env.MCP_HTTP_KEY ?? "";
  if (expected) {
    const header = c.req.header("authorization") ?? "";
    const match = header.match(/^Bearer\s+(.+)$/i);
    const token = match?.[1]?.trim() ?? new URL(c.req.url).searchParams.get("key") ?? "";
    if (token !== expected) return c.json({ error: "unauthorized" }, 401);
  }

  let msg: JsonRpcMessage;
  try {
    msg = await c.req.json<JsonRpcMessage>();
  } catch {
    return c.json({ jsonrpc: "2.0", id: null, error: { code: -32700, message: "Parse error" } });
  }

  try {
    const out = await handleMcp(msg, c.env, app);
    if (out === null) return new Response(null, { status: 204 });
    return c.json(out);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return c.json({ jsonrpc: "2.0", id: msg.id ?? null, error: { code: -32000, message } });
  }
});
